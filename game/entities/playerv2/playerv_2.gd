...
func _on_opening_animation_finished(anim_name: StringName) -> void:
	if anim_name == "get_up":
		_is_input_locked = false
		_animation_player.animation_finished.disconnect(_on_opening_animation_finished)
...
func _resolve_interaction_target() -> void:
	...
	for interactable: Interactable in _nearby_interactables:
		...
		if dot > 0.3:  # ~72° cone — adjust to taste.
...
func _handle_interact_input() -> void:
	...
	# dialogue_advance only fires when the target is actively displaying dialogue.
