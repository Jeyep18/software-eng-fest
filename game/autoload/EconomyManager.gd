...
func purchase(item_data: ItemData) -> bool:
	...
	if item_data.item_price <= 0:
		...
		_debug_log("EconomyManager: '%s' is not purchasable." % item_data.item_name)
		return false
	...
	if not GameState.spend_cash(item_data.item_price):
		...
		_debug_log("EconomyManager: Not enough cash. Need ₱%d, have ₱%d" % [
	...
	if not InventoryManager.can_accept_item(item_data):
		...
		_debug_log("EconomyManager: Inventory full. Refunding ₱%d." % item_data.item_price)
		return false
	...
	_debug_log("EconomyManager: Purchased '%s' for ₱%d. Remaining cash: ₱%d" % [
...
func barter(npc_id: String, give_item_id: String, receive_item_data: ItemData) -> bool:
	...
	if state == "completed":
		...
		_debug_log("EconomyManager: Barter with '%s' already completed." % npc_id)
		return false
	if state == "expired":
		barter_failed.emit(give_item_id, "Trade has expired.")
		_debug_log("EconomyManager: Barter with '%s' has expired." % npc_id)
		return false
	...
	if not InventoryManager.has_item_with_id(give_item_id):
		...
		_debug_log("EconomyManager: Player missing required trade item: '%s'" % give_item_id)
		return false

	# Guard: inventory full for received item
	if InventoryManager.is_full():
		barter_failed.emit(give_item_id, "Inventory full.")
		_debug_log("EconomyManager: Inventory full. Cannot receive '%s'." % receive_item_data.item_name)
		return false
	...
	_debug_log("EconomyManager: Barter complete with '%s'. Gave '%s', received '%s'." % [
